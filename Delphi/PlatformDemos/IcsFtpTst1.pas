...
type
  ...
  TFtpReceiveForm = class(TForm)
    ...
    Label15: TLabel;
    Lanugage: TEdit;
    SettingsPageControl: TTabControl;
    MainSettingsTabSheet: TTabItem;
    Panel2: TPanel;
    Label1: TLabel;
    Label3: TLabel;
    Label4: TLabel;
    ...
    ConnectionTypeTabSheet: TTabItem;
    Label22: TLabel;
...
begin
    if not FInitialized then begin
        FInitialized := TRUE;
        SettingsPageControl.ActiveTab :=  SettingsPageControl.Tabs[0];
        OptsEdit.ItemIndex := 0;
        ...
        DataPortRangeEndEdit.Text :=
                              IniFile.ReadString(SectionData, KeyPortEnd,
                                                 '0');
        PassiveCheckBox.IsChecked        := Boolean(IniFile.ReadInteger(SectionData, KeyPassive , 0));
        DisplayCheckBox.IsChecked        := Boolean(IniFile.ReadInteger(SectionData, KeyDisplay , 0));
        SyncCheckBox.IsChecked           := Boolean(IniFile.ReadInteger(SectionData, KeySync    , 0));
        NoAutoResumeAtCheckBox.IsChecked := Boolean(IniFile.ReadInteger(SectionData, KeyNoResume, 0));
        BinaryCheckBox.IsChecked         := Boolean(IniFile.ReadInteger(SectionData, KeyBinary,   0));
      ...
      {$ELSE}
        Panel2.StyleLookup := 'PanelMacStyle';
      {$ENDIF}
    end;
end;
...
begin
    ...
    IniFile.WriteInteger(SectionData, KeyPassive , Ord(PassiveCheckBox.IsChecked));
    IniFile.WriteInteger(SectionData, KeyDisplay , Ord(DisplayCheckBox.IsChecked));
    IniFile.WriteInteger(SectionData, KeySync    , Ord(SyncCheckBox.IsChecked   ));
    IniFile.WriteInteger(SectionData, KeyNoResume, Ord(NoAutoResumeAtCheckBox.IsChecked));
    IniFile.WriteInteger(SectionData, KeyBinary,   Ord(BinaryCheckBox.IsChecked));
...
begin
    ...
    if FLastProgress < IcsGetTickCount then begin
        FLastProgress := IcsGetTickCount + 1000;
...
begin
    ...
    if DisplayCheckBox.IsChecked then
        exit;
    try
...
begin
    StateLabel.Text := IntToStr(Ord(FtpClient1.State));
end;
...
begin
    ...
    FtpClient1.DisplayFileFlag    := DisplayCheckBox.IsChecked;
    ...
    if bandwidth > 0 then begin
        ...
        FtpClient1.Options := FtpClient1.Options - [ftpBandwidthControl];
    FtpClient1.DataSocketSndBufSize := 8192 * 4;
    FtpClient1.DataSocketRcvBufSize := 8192 * 4;
    if SyncCheckBox.IsChecked then begin
        if SyncCmd then
...
begin
    ...
    FtpClient1.Passive         := PassiveCheckBox.IsChecked;
...
begin
    ...
    FtpClient1.HostFileName    := HostFileEdit.Text;
    FtpClient1.LocalFileName   := LocalFileEdit.Text;
    FtpClient1.Binary          := BinaryCheckBox.IsChecked;
    FtpClient1.Passive         := PassiveCheckBox.IsChecked;
...
begin
    ...
    FtpClient1.LocalFileName   := LocalFileEdit.Text;
    FtpClient1.Options         := [];
    if NoAutoResumeAtCheckBox.IsChecked then
...
begin
    if FRunning then begin
        ...
        StressPutButton.Text := 'Stress Put';
        Exit;
    end;
    FRunning := TRUE;
    try
        Display('Stress Put started.');
        StressPutButton.Text := 'STOP';
        Count := 0;
...
begin
    FtpClient1.NewOpts    := OptsEdit.Selected.Text;
