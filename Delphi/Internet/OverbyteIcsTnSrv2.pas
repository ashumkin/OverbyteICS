...
type
  ...
  private
    ...
    FRcvdCR   : Boolean;
    FDataFlag : Boolean;
    procedure ProcessChar(Ch : AnsiChar);
...
begin
    ...
    { Skip leading spaces }
    I := 1;
    while (I <= Length(CommandVerb)) and IsCharInSysCharSet(CommandVerb[I], [' ', #9]) do
        Inc(I);
    ...
    while TRUE do begin
        ...
        if IsCharInSysCharSet(CommandVerb[J], [' ', #9, '/']) then begin
...
procedure TClientForm.ProcessChar(Ch : AnsiChar);
begin
    if Ch = #8 then begin
