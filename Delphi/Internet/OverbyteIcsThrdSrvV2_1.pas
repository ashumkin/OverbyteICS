...
Description:  Basic TCP server showing how to use TWSocketServer and
              ...
              Version is similar to version 1 with the difference that all
              communications for a client is done in the context of the
              worker thread. In version 1 demo, the banner was sent in the
...
type
  ...
  private
    ...
    procedure WMUnleashThread(var Msg: TMessage); message WM_UNLEASH_THREAD;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TTcpSrvForm.WMUnleashThread(var Msg: TMessage);
var
    Client : TThrdSrvClient;
begin
    Client := (TObject(Msg.LParam) as TThrdSrvClient);
    LeaveCriticalSection(Client.ClientThread.FCritSect);
end;
...
begin
    with Client as TThrdSrvClient do begin
        ...
        { Acquire critical section to block thread after init               }
        EnterCriticalSection(ClientThread.FCritSect);
...
constructor TClientThread.Create(Suspended: Boolean);
begin
    inherited;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// StartConnection is called in the context of the main thread
// We want code to be executed in the context of the worker thread.
// When StartConnection is called, the worker thread is waiting on the
// critical section.
// While it is waiting, we will post a message to his message queue so that
// when it is unblocked he will proceed with start connection in his context.
// We also want the main thread to be out of the session available event
// handler so we post a message to the application form and from the
// corresponding event handler, we will unblock the worker thread by leaving
// the critical section.
procedure TThrdSrvClient.StartConnection;
begin
    PostMessage(Handle, WM_START_CNX, 0, 0);
    PostMessage(TcpSrvForm.Handle, WM_UNLEASH_THREAD, 0, Integer(Self));
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TThrdSrvClient.WndProc(var MsgRec: TMessage);
begin
    case MsgRec.Msg of
    WM_START_CNX: WMStartCnx(MsgRec);
    else
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// This will be extecuted in the worker thread context after the thread has
// been unblocked waiting to acquire the critical section.
procedure TThrdSrvClient.WMStartCnx(var Msg: TMessage);
begin
    if Length(FBanner) > 0 then
        SendStr(AnsiString(FBanner) + FLineEnd);
end;
